...
**Exception**: `ctrl+tab` and `shift+ctrl+tab` cannot be bound to a custom app shortcut. macOS's key view loop intercepts both keys before Veil's normal dispatch chain runs, and Veil currently routes the events straight to Neovim rather than override the system's handling. Binding an app shortcut to either key is silently ignored.
